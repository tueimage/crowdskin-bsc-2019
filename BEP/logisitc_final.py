# -*- coding: utf-8 -*-
"""
Created on Fri Jun 14 14:04:43 2019

@author: s163729
"""
#scrip logistic regression 
import pandas as pd 
...
for i in range(0,11):
    ...
    #apply logistic regressiono 
    logisticRegr= LogisticRegression() 
    logisticRegr.fit(x_train, y_train)  
    
    #predict classes
    predictions = logisticRegr.predict(x_test) 
    predictions2 = logisticRegr.predict(x_train)       
    ...
    #measure sensititvy, specificity and precision 
...
for i in range(0,11):
    ...
    #measure sensitivy, specificity and precision 
    conf = confusion_matrix(y_test, predictions)
    conf2 = confusion_matrix(y_train, predictions2)
...
asym_id= pd.concat([data1['ID'],data2['ID'], data3['ID'], data4['ID'], data5['ID'], data6['ID'], data7['ID'],data8['ID']], axis = 0 )
...
for i in range(0,11):
    ...
    for train_index, test_index in sss.split(asym_label,truth):
        x_train, x_test = df(asym_label.iloc[train_index]), df(asym_label.iloc[test_index])
...
for i in range(0,11):
    ...
    #measure sensitivty, specificity and precsion 
    conf = confusion_matrix(y_test, predictions)
    conf2 = confusion_matrix(y_train, predictions2)
...
plt.ylabel('AUC') 
plt.title('AUC Logistic Regression, test set') 
